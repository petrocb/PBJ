...
class NewSMACross():
    def __init__(self, cond):
        ...
        self.sellOpen = False
        if cond[0] > cond[1]:
            self.closes = functions.startPastPricesList(self.cond[0], 'EUR_USD', "M1", "SMACrossOver")
        else:
            self.closes = functions.startPastPricesList(self.cond[1], 'EUR_USD', "M1", "SMACrossOver")
        self.short_ema = [functions.sma(self.closes[-self.cond[0]:])]
        self.long_ema = [functions.sma(self.closes[-self.cond[1]:])]
        self.id = 0
        self.direction = functions.startPastPricesList(60, 'EUR_USD', "M1", "SMACrossOver")

    def tick(self):

        functions.checkSLnTP()
        if functions.getPositions("SMACrossOver")['positions'] == []:
            self.buyOpen = False
            self.sellOpen = False

        if self.cond[0] > self.cond[1]:
            self.closes = functions.updatePastPrices(self.closes, self.cond[0], 'EUR_USD', "M1", "SMACrossOver")
        else:
            self.closes = functions.updatePastPrices(self.closes, self.cond[0], 'EUR_USD', "M1", "SMACrossOver")
        ...
        if len(self.long_ema) > 2:
            self.long_ema.pop(0)
        self.direction = functions.updatePastPrices(self.direction, 60, 'EUR_USD', "M1", "SMACrossOver")
        ...
        if self.short_ema[1] < self.long_ema[1] and self.short_ema[0] >= self.long_ema[
            ...
            # print("!!!!CLOSE!!!!")
            self.id = functions.close(self.id, "SMACrossOver")
            self.sellOpen = False
            self.buyOpen = False
