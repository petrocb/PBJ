...
class SMAFollowTrend:
    def __init__(self):
        self.id = 0
        self.data = functions.startPastPricesList(4097, "EUR_USD", "M30", "test")
        self.direction = 0
        self.position = 0
    ...
    def tick(self):
        ...
        functions.update("test")
        self.data = functions.updatePastPrices2(self.data, 4097, "EUR_USD", "M30", "test")
        ...
        self.direction *= 500
        self.position = functions.getPositions("test")['positions']
        if self.position:
        ...
        if self.position != self.direction:
            functions.order(float(self.direction) - float(self.position), "test", "test", 0.001, 0.001, 0)
